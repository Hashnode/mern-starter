...
test.serial('Send text message with trilio and required parameters will successfully send the message', t => {
  notification.sendTextMessage(testMessage, recipient, sender, (error, response) => {
    t.is(error, null, `error is not null, but ${this.error}.`);
    t.not(response, null, `Response is not null, but ${this.response}.`);
  });
});
